...
void ACMH::InuputInitialization(const std::string &dense_folder, const Problem &problem)
{
    images.clear();
    cameras.clear();
    ...
    if (params.geom_consistency) {
        ...
        for (size_t i = 0; i < num_src_images; ++i) {
            std::stringstream result_path;
            result_path << dense_folder << "/ACMH" << "/2333_" << std::setw(8) << std::setfill('0') << problem.src_image_ids[i];
            std::string result_folder = result_path.str();
            std::string depth_path = result_folder + "/depths.dmb";
            cv::Mat_<float> depth;
...
void ACMH::CudaSpaceInitialization(const std::string &dense_folder, const Problem &problem)
{
...
int ACMH::GetReferenceImageWidth()
{
    return cameras[0].width;
}

int ACMH::GetReferenceImageHeight()
{
    return cameras[0].height;
}

cv::Mat ACMH::GetReferenceImage()
{
    return images[0];
}

float4 ACMH::GetPlaneHypothesis(const int index)
{
    return plane_hypotheses_host[index];
}

float ACMH::GetCost(const int index)
{
